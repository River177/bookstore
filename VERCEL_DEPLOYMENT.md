...
### 1. 构建失败：Cannot find package '@vitejs/plugin-vue'
**错误信息**：
```
Error: Cannot find package '@vitejs/plugin-vue' imported from ...
```

**原因**：构建时需要的包被放在 `devDependencies` 中，但 Vercel 构建时需要它们

**解决方案**：确保以下构建相关的包在 `dependencies` 中（不是 `devDependencies`）：
- `@vitejs/plugin-vue`
- `@tailwindcss/vite`
- `vite`
- `tailwindcss`
...
这个问题已在最新的 `package.json` 中修复。如果仍有问题，请重新安装依赖：
```bash
rm -rf node_modules package-lock.json
npm install
```

### 2. 部署失败：Prisma Client 未生成
